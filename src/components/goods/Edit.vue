...
export default {
  ...
  methods: {
    // 获取所有商品信息
    async getGood () {
      const { data: res } = await this.$http.get('goods/' + this.$route.params.id, { params: { id: this.$route.params.id } })
    ...
    add () {
      this.$refs.addFormRef.validate(async valid => {
        ...
        // 发起请求
        const { data: res } = await this.$http.put('goods/' + this.$route.params.id, form)
        if (res.meta.status !== 200) {
